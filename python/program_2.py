...
def shortest_path_prompt(network):
    ...
    try: 
        ...
        start_stop = public_transport.Stop(str(raw_input("Podaj przystanek początkowy: ")).strip())

        if start_stop not in network.vertices(): 
            print("Podany przystanek nie istnieje!\n")
            return 

        end_stop = public_transport.Stop(str(raw_input("Podaj przystanek końcowy: ")).strip())
...
def hamiltonian_cycle_prompt(network):
    ...
    try: 
        no_of_stops = int(raw_input("Podaj liczbe przystanków przez które wyznaczyć min trasę:"))

        for i in range(no_of_stops):
            next_stop = public_transport.Stop(str(raw_input("Podaj %d początkowy: " % (i+1))).strip())
...
while True:
    ...
    try: 
        menu_option = int(raw_input("Wybierz opcję wyszukiwania: "))
    ...
    if menu_option == SHORTEST_PATH:
        shortest_path_prompt(network)
    elif menu_option == HAMILTONIAN_CYCLE: 
