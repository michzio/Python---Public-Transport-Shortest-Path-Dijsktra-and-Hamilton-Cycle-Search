...
class Dijkstra: 
	""" Klasa implementująca algorytm Dijkstry 
		...
		def min_path(source, target):
			...
			if self.path_cost[target] == INFINITY: 
		...
		def find_min_paths_from(source): 
			...
			while queue: 
				...
				if self.path_cost[node] == INFINITY: 
